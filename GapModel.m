function [mN mL mS Net Lnet Snet rTransect r w vN vLn vSn Sin Lin mSin mLin percBelowOpen R_open S_open L_open OpenAll GapAll Sdir_open Sdif_open mSdir mSdif]=GapModel(H,HeightMap,R,Slope,Aspect,Site,plotoff,SVF)
NoC=20;
...
if exist([Site.Name '\temperature.txt'],'file') == 2
    ...
    [Tair Tsnow]=GapTemperature(DOYPeriod',Site.T_m,Site.T_yrange,Site.T_drange,Site.T_phase,Site.RH(mod(ceil(DOYPeriod),365)+1),Site);
end

Sdir=zeros(size(SVF));
Sdif=zeros(size(SVF));
Ssnow=zeros(size(SVF));
...
for DOY=DOYPeriod;
   ...
   if Sun.Alpha>0
       EXP_MU_PL_H=exp(-Site.Miu*PL*H);
       Sdir=Sdir+Sun.Sdiropen*EXP_MU_PL_H;
       Sdif=Sdif+Sun.Sdifopen*SVF;
      %[DOY Sun.Sdifopen Sun.Sdiropen]
       Sdir_open=Sdir_open+Sun.Sdiropen;
       Sdif_open=Sdif_open+Sun.Sdifopen;
       
       if Site.albedo_temporal_exists
            albedo=Site.albedo_temporal(ceil(mod(DOY,365)));
            albedo_angle_corrected=albedo+0.53*albedo*(1-albedo)*(1-cosd(Sun.Theta)).^1.2;
            Ssnow=Ssnow+albedo_angle_corrected*(Sun.Sdiropen*EXP_MU_PL_H)+albedo*(Sun.Sdifopen*SVF);
            Ssnow_open=Ssnow_open+albedo_angle_corrected*Sun.Sdiropen+albedo*Sun.Sdifopen;
            Snet=Snet+ (1-albedo_angle_corrected)./(1-albedo_angle_corrected*Site.albedo_canopy*(1-SVF)).*(Sun.Sdiropen.*EXP_MU_PL_H) + (1-albedo)./(1-albedo*Site.albedo_canopy*(1-SVF)).*(Sun.Sdifopen*SVF);
       end
   end
end
...
%dT=2/150*Sinc;
dT=Site.delTc_t/Site.delTc_ins*Sinc;
...
CVF=GapLocalCVFDirectional(R,r,w,Slope,Site.NW);

meanTair=mean(Tair);

Tcan4=zeros(Site.NW+1,Site.NR);
%dTMap=zeros(Site.NW+1,Site.NR);
%dTMap4=zeros(Site.NW+1,Site.NR);
for i=1:Site.NW+1
    for j=1:Site.NR
        %dTMap(i,j)=sum(dT.*1./distance(:,i,j)./sum(1./distance(:,i,j)));
        Tcan4(i,j)=sum(((dT+meanTair).^4).*CVF(:,i,j)./distance(:,i,j))./sum(CVF(:,i,j)./distance(:,i,j));
    end
end
%dTMap=dTMap4.^0.25;
...
mSdir=MeanVarWeighted(Sdir,Area);
mSdif=MeanVarWeighted(Sdif,Area);
...
elseif plotoff==4
    fontsize=18;
    ContourPolar(r,w,Net,['Net= ' num2str(mN(1),'%.1f') ' W/m^2'],NoC,'auto',Aspect,fontsize);
elseif plotoff==-1
    fontsize=8;
    subplot(2,2,1),ContourPolar(r,w,Snet,['S_N_e_t= ' num2str(mS(1),'%.1f') ' W/m^2 ' ' S_o_p_e_n= ' num2str(S_open,'%.1f') ' W/m^2' ' \DeltaS= ' num2str(mS(1)-S_open,'%.1f') ' W/m^2'],NoC,'auto',Aspect,fontsize);
    subplot(2,2,2),ContourPolar(r,w,Lnet,['L_N_e_t= ' num2str(mL(1),'%.1f') ' W/m^2 ' ' L_o_p_e_n= ' num2str(L_open,'%.1f') ' W/m^2' ' \DeltaL= ' num2str(mL(1)-L_open,'%.1f') ' W/m^2'],NoC,'auto',Aspect,fontsize);
    subplot(2,2,3),ContourPolar(r,w,Net,['Net= ' num2str(mN(1),'%.1f') ' W/m^2 ' ' R_o_p_e_n= ' num2str(R_open,'%.1f') ' W/m^2' ' \DeltaR= ' num2str(mN(1)-R_open,'%.1f') ' W/m^2'],NoC,'auto',Aspect,fontsize);
    subplot(2,2,4),ContourPolar(r,w,Net>R_open,'',NoC,'auto',Aspect,fontsize);
elseif plotoff==3
