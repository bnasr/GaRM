function [mN mL mS Net Lnet Snet rTransect r w vN vLn vSn Sin Lin mSin mLin percBelowOpen R_open S_open L_open OpenAll GapAll]=GapModel3(H,HeightMap,R,Slope,Aspect,Site,plotoff,SVF)
NoC=20;
...
Lcan_Map=zeros(Site.NW+1,Site.NR,length(Tair));
for i=1:Site.NW+1
    for j=1:Site.NR
        cvf=CVF(:,i,j);
        invDistance=1./distance(:,i,j);
        for t=1:length(Tair)
            %dTMap(i,j)=sum(dT.*1./distance(:,i,j)./sum(1./distance(:,i,j)));
            %dTMap(i,j)=sum(dT.*TVFDirectional(:,i,j)./sum(TVFDirectional(:,i,j)));
            %Lcan_Map(i,j,t)=mean((Tair(t)+ dT(:)).^4*CVF(:,i,j));

            Lcan_Map(i,j,t)=MeanVarWeighted((Tair(t)+ dT(:)).^4.*cvf(:),invDistance(:));
            %Lcan_Map(i,j,t)=mean((Tair(t)+ dT(:)).^4.*cvf(:));
        end
    end
    i
end

Lcan_Map=StephanBoltzman*Site.eCan.*Lcan_Map;
Lcan=mean(Lcan_Map,3);
